...
class _DashboardViewState extends State<DashboardView>
  ...
  Future<void> _handleEventResponse(Events event, String response) async {
    // Store the old response outside the try block, handling null case
    final String? oldResponse = _userResponses[event.id];
    ...
    try {
      ...
      setState(() {
        ...
        // Update participant count - add null check for oldResponse
        if (oldResponse == 'Going' && response != 'Going') {
          _participantCounts[event.id] =
              (_participantCounts[event.id] ?? 0) - 1;
        } else if (oldResponse != 'Going' && response == 'Going') {
          _participantCounts[event.id] =
              (_participantCounts[event.id] ?? 0) + 1;
        }
      });
      ...
      // Revert local state on error, handling the case where oldResponse might be null
      setState(() {
        if (oldResponse != null) {
          _userResponses[event.id] = oldResponse;
        } else {
          _userResponses.remove(
              event.id); // Remove the entry if there was no previous response
        }
      });
...
${event.hasImage ? "\nImage: ${event.imageUrl}" : ""}''';
  ...
  Widget _buildEventsList(
      List<Events> events, String emptyMessage, bool isActive) {
      ...
      child: events.isEmpty
          ? Center(
              child: Column(
                ...
                children: [
                  // Lottie animation based on the tab
                  SizedBox(
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: TabBarView(
        controller: _tabController,
        children: [
          _buildEventsList(activeEvents, 'No active events', true),
          _buildEventsList(upcomingEvents, 'No upcoming events', false),
        ],
      ),
    );
  }
