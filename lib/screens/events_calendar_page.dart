...
enum EventStatus { all, published, draft, recent }
  ...
  List<Events> _filterEventsByStatus(List<Events> events) {
    ...
    switch (_selectedStatus) {
      ...
      case EventStatus.draft:
        return events.where((event) => event.status == 'draft').toList();
...
class _EventDetailsDialogState extends State<EventDetailsDialog> {
  Map<String, List<String>> _attendees = {
    'Going': [],
    'Not Going': [],
  };
  ...
  Future<void> _fetchAttendees() async {
    try {
      ...
      final authProvider = Provider.of<AuthProvider>(context, listen: false);
      final responses =
          await authProvider.getEventResponses([widget.event.id.toString()]);

      // Group attendees by response
      Map<String, List<String>> attendees = {
        'Going': [],
        'Not Going': [],
      };

      for (var entry in responses.entries) {
        if (entry.value == 'Going') {
          attendees['Going']!.add(entry.key);
        } else if (entry.value == 'Not Going') {
          attendees['Not Going']!.add(entry.key);
        }
      }
      ...
      if (mounted) {
        setState(() => _isLoading = false);
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Error fetching attendees: ${e.toString()}')),
        );
      }
    }
  }
  ...
  Widget _buildAttendeesList(String status) {
    final List<String> attendees = _attendees[status] ?? [];
    ...
    return Column(
      ...
      children: [
        ...
        if (attendees.isEmpty)
          Text(
            'No attendees ${status.toLowerCase()}',
            style: TextStyle(
              color: Colors.grey[600],
